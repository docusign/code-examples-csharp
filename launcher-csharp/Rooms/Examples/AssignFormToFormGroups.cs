...
namespace DocuSign.Rooms.Examples
{
    public class AssignFormToFormGroups
    {
        ...
        public static (FormSummaryList forms, FormGroupSummaryList formGroups) GetFormsAndFormGroups(string basePath, string accessToken, string accountId)
        {
        ...
        public static FormGroupFormToAssign AssignForm(string basePath, string accessToken, string accountId, string formGroupId, FormGroupFormToAssign formToAssign)
        {
            // Construct your API headers
            var apiClient = new ApiClient(basePath);
            apiClient.Configuration.DefaultHeader.Add("Authorization", "Bearer " + accessToken);
