...
namespace DocuSign.Rooms.Examples
{
    public class GrantOfficeAccessToFormGroup
    {
        ...
        public static (OfficeSummaryList offices, FormGroupSummaryList formGroups) GetOfficesAndFormGroups(string basePath, string accessToken, string accountId)
        {
        ...
        /// <returns></returns>
        public static void GrantAccess(string basePath, string accessToken, string accountId, string formGroupId, int? officeId)
        {
            // Construct your API headers
            var apiClient = new ApiClient(basePath);
            apiClient.Configuration.DefaultHeader.Add("Authorization", "Bearer " + accessToken);
